        ...
        <div id="card-drawel" class="card card-info">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="ml-auto d-flex">      
                        <a href="{{ route('ordenes.ingreso', $ordenes['infoOrden']->num_orden_compra) }}" 
                          class="btn btn-info mx-1">
                          <i class="fa fa-file-excel"></i> Ingreso
                        </a>
    ...
    <div style="display: flex; justify-content: space-between; gap: 1rem;">
        ...
        <div class="card-body"> 
            ...
            <p id="total-orden"><strong>Total Orden:</strong> {{ number_format($ordenes['infoOrden']->total, 2) }}</p>
              ...
              @if(number_format($ordenes['Diferencia'], 2) == 0)
              <p id="valor-diferencia" style="background-color: green; color: white;"><strong>Diferencia:</strong> No hay diferencia</p>       
              @endif

              @if(number_format($ordenes['Diferencia'], 2) < 0)
              <p id="valor-diferencia" style="background-color: blue; color: white;"><strong>Diferencia:</strong> {{number_format($ordenes['Diferencia'], 2)}} Menor al valor pactado </p>       
              @endif

              @if(number_format($ordenes['Diferencia'], 2) > 0)
              <p id="valor-diferencia" style="background-color: red; color: white;"><strong>Sobrecosto:</strong>  {{number_format($ordenes['Diferencia'], 2)}}</p>       
              @endif
        ...
        btn.addEventListener('click', function () {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const Entregada = parseInt(data.CantidadEntregada);
                    
                    const cell       = row.querySelector('.editable-cell');
                    const viewSpan   = cell?.querySelector('.view-mode');
                    const editInput  = cell?.querySelector('.cantidad-entregada-input');
                    const entregada  = parseInt(data.CantidadEntregada, 10) || 0;
                    const faltantes = cantidadTotal - parseInt(entregada);
                    row.querySelector('.faltantes-cell').textContent = faltantes;

                    if (viewSpan)  viewSpan.textContent = entregada; // muestra acumulado
                    if (editInput) editInput.value      = 0;         // opcional: resetea input
                    ...
                    {
                         $('#valor-diferencia').html(
                      '<p style="background-color: green;"><strong style="background-color: green;">Diferencia: No Hay Diferencia</strong></p>');
                    }
                    else if(data.Diferencia < 0)
                    {
                        $('#valor-diferencia').html(

                      '<p style="background-color: blue;"><strong style="background-color: blue;">Diferencia: ' + data.Diferencia.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' Menor al valor pactado </strong></p>');
                    }
                    else if(data.Diferencia > 0)
                    {
                        $('#valor-diferencia').html(
                      '<p style="background-color: red;"><strong style="background-color: red;">Diferencia: ' + data.Diferencia.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' Sobrecosto </strong></p>');
                    }
