    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="card col-l-12">
                <div class="card-header bg-warning">{{ __('Entregados y form para realizar el reporte de lo entregado') }}</div>
                    ...
                    <thead>
                       ...
                       <tbody>
                        ...
                        @foreach ($item3 as $item)
                         @if(!in_array($item['EstEntrega'] ?? '', [0,2]))
                        <tr>
    ...
    $("tbody tr").each(function(el){
          ...
          $.ajax({
           ...
           // $('#mipres').DataTable().destroy();
            ...
            const datar = JSON.stringify(data.result);
                
             $.each(JSON.parse(data.result), function(i, item) {
                  console.log(item);
                   /*  $.each(JSON.parse(item), function(i, items) {
                                    var errores = [];
                                    errores.push(items + '<br>');
...
$("#reportare").click(function(){
    ...
    if($("#customRadio1").prop('checked')){
        ...
        causano = $("#CausaNoEntrega").val();
        
        if(causano == '' || causano == null)
        {
            causano = 0;
        }else{
            causano = $("#CausaNoEntrega").val();
            ...
            itemmipres.CausaNoEntrega = causano;
